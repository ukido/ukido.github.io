...
import { Language, Languages, MediaBreakPoint, NavbarItems, Theme, Themes } from "@app/data/model/ui.model";
...
@Component({
    selector: 'app-navbar',
    standalone: true,
    imports: [RouterLink, MatListModule, MatButtonModule, MatIconModule, MatSlideToggleModule,TranslateModule],
...
export class AppNavbarComponent {
    ...
    @Input()
    screen: MediaBreakPoint | null = 'small';

    @Output()
    openDrawer: EventEmitter<void> = new EventEmitter();

    @Output()
    switchTheme: EventEmitter<void> = new EventEmitter();
  ...
  ngAfterViewInit() {
    if (this.element){
      this.element.nativeElement.querySelector('.mdc-switch__icon--on').firstChild.setAttribute('d', sun);
      this.element.nativeElement.querySelector('.mdc-switch__icon--off').firstChild.setAttribute('d', moon);
    }
  }
