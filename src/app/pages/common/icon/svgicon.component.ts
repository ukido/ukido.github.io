...
const UNKNOWN_ICON = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M11 4h2v11h-2zm2 14v2h-2v-2z"/></svg>`;

const INSTAGRAM = `<svg xmlns="http://www.w3.org/2000/svg" 
...
export class AppSvgIconDirective implements OnChanges {

    @Input() appSvgIcon = '';
    @HostBinding('style.height.px') @Input() height = 24;
    @HostBinding('style.width.px') @Input() width = 24;
    @HostBinding('style.fill') @Input() fillColor = 'currentColor';
    ...
    constructor(private el: ElementRef) {
        // this.addIconStyle(this.el.nativeElement)
    }

    ngOnChanges(changes: SimpleChanges): void {
        const iconChanges = changes['appSvgIcon'];
        if (iconChanges) {
            if (SUPPORTED_ICONS.has(iconChanges.currentValue)) {
                this.el.nativeElement.innerHTML = SUPPORTED_ICONS.get(iconChanges.currentValue);
            } else {
                this.el.nativeElement.innerHTML = UNKNOWN_ICON;
            }
        }
    }


    private addIconStyle(el: HTMLElement) {
